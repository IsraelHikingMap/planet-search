...
The merge cannot finish until the last segment has streamed by, and there are tens of millions of streets on the planet, so the build holds only two longs per street while it runs — the winning way id and its point — and no documents. The documents are built at the end, from a second read of the `.osm.pbf` that decodes only the ways that won. That read needs no node locations, since every street's point was already recorded, so it is a sequential scan rather than a second planetiler pass. It does mean the input file has to still be there when the build finishes: `free_osm_after_read=true` would delete it and the streets would go unindexed, with an error in the log.
